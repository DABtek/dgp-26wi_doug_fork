...
public class CatAttacker extends BabyRat {
        ...
        for (RobotInfo info : nearbyInfos) {
            if (info.getType().isCatType()) {
                ...
                for (Message m : squeakMessages) {
                    int msg = m.getBytes();
                    SqueakType msgType = getSqueakType(msg);
                    if (msgType == SqueakType.CAT_FOUND) {
                        ...
                        System.out.println("Received msg type " + msgType.toString());
                    }
                }
    ...
    public void runReportBack() throws GameActionException {
        ...
        } else if (rc.canSenseLocation(forwardLoc)) {
